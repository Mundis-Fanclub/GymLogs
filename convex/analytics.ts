...
export const getWorkoutFrequency = query({
  ...
  handler: async (ctx, args) => computeWorkoutFrequency(ctx, args),
});

async function computeWorkoutFrequency(
  ctx: QueryCtx,
  args: {
    userId: Id<"users">;
...
export const getMuscleAnalytics = query({
  ...
  handler: async (ctx, args) => computeMuscleAnalytics(ctx, args),
});

async function computeMuscleAnalytics(
  ctx: QueryCtx,
    ...
    if (set.createdAt >= currentStart) {
      const zones =
        exercise?.bodygraphZones && exercise.bodygraphZones.length > 0
          ? exercise.bodygraphZones.map((z) => toBodyPart(z) as BodyPart)
          : [bodyPart];
...
export const getAnalyticsOverview = query({
  ...
  handler: async (ctx, args) => {
    const [muscleAnalytics, workoutFrequency] = await Promise.all([
      computeMuscleAnalytics(ctx, args),
      computeWorkoutFrequency(ctx, { userId: args.userId, period: "week" }),
    ]);
